...
options obs=100 validvarname=any;

/* The upstream script reads "Breast Cancer METABRIC.csv" via PROC IMPORT.  */
/* For a self-contained run we inline a small sample of the same columns    */
/* the script references, keeping the original variable names verbatim.     */
data raw_metabric;
  infile datalines dsd dlm='|' truncover;
  length 'Patient ID'n $12 Sex $8 'ER Status'n $8 'HER2 Status'n $8
         Chemotherapy $4 'Overall Survival Status'n $12;
  input 'Patient ID'n $
        'Age at Diagnosis'n
        Sex $
        'Tumor Stage'n
